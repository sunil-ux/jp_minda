...
export class TerritoryAddComponent implements OnInit {
  ...
  constructor(public router:Router,public service:MyserviceService,public browser:BrowserAnimationsModule, public dialog:DialogBoxService,public toaster:ToastrManager) 
  {
    this.get_state();
  }
  ...
  onsubmit()
  {
    ...
    if(this.finalPincodesDetailArr && this.finalPincodesDetailArr.length && this.form.territory_name){
      this.valid=true;
      console.log(this.form);
      this.service.get_data(this.form,'Territory_master/add_territory').subscribe((result)=>
      {
        if(result['data'])
        {
          ...
          this.router.navigate(['/territory-master/']);
        }
        else
        {
